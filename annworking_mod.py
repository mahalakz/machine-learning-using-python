...
dataset = pd.read_csv('D:/desktop/AI Training Content/Churn_Modelling.csv')
...
z=pd.DataFrame(X[:,[0,2,3,4,5,6,7,8,9]])

onehotencoder=OneHotEncoder(handle_unknown='ignore')
ip=pd.DataFrame(X[:,1],columns=['test'])
b = pd.DataFrame(onehotencoder.fit_transform(ip[['test']]).toarray(),columns=['10','11','12'])
X=z.join(b)
